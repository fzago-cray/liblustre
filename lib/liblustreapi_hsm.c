 ...
 * \retval -errno on error.
 */
static int llapi_hsm_log_ct_registration(struct hsm_copytool_private *ct,
					 __u32 event_type)
{
	int				rc;
				     ...
				     __u64 total, __u64 current)
{
	...
	if (progress_type != CT_FINISH) {
		rc = llapi_fid2path(hcp->ct_priv->lfsh, &hai->hai_fid,
				    lustre_path,
				    sizeof(lustre_path), NULL, NULL);
		if (rc < 0)
			goto err;
...
static int ct_stat_by_fid(const struct lustre_fs_h *lfsh,
			  ...
			  struct stat *buf)
{
	...
	rc = fstatat(lfsh->fid_fd, fid_name, buf, 0);
	return rc ? -errno : 0;
}
			   ...
			   bool is_error)
{
	...
	if (hai->hai_action == HSMA_RESTORE) {
		rc = ct_stat_by_fid(hcp->ct_priv->lfsh,
				    &hai->hai_fid, &hcp->stat);
		if (rc < 0)
			goto err_out;
...
int llapi_hsm_action_get_fd(const struct hsm_copyaction_private *hcp)
{
	...
	if (hai->hai_action == HSMA_ARCHIVE) {
		fd = llapi_open_by_fid(hcp->ct_priv->lfsh, &hai->hai_dfid,
