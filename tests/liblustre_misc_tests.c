...
void unittest_llapi_parse_size(void)
{
	...
	/* byte */
	size_units = 0;
	rc = llapi_parse_size("1453", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
	ck_assert_int_eq(size, 1453);
	ck_assert_int_eq(size_units, 1);

	size_units = 0;
	rc = llapi_parse_size("145", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
	...
	/* b is block, not byte.*/
	size_units = 0;
	rc = llapi_parse_size("54353b", &size, &size_units, 0);
	ck_assert_int_eq(rc, 0);
	...
	size_units = 0;
	rc = llapi_parse_size("145353452354354353b", &size, &size_units, 0);
	...
	for (i=0; i<strlen(ext); i++) {
		...
		size_units = 0;
		rc = llapi_parse_size(args, &size, &size_units, 0);
		ck_assert_int_eq(rc, 0);
		...
		size_units = 0;
		rc = llapi_parse_size(args, &size, &size_units, 1);
		ck_assert_int_eq(rc, 0);
	...
	/* With leading spaces */
	size_units = 0;
	rc = llapi_parse_size("    87", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
	...
	size_units = 300;
	rc = llapi_parse_size("17", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
	...
	size_units = 0;
	rc = llapi_parse_size("hello", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);

	size_units = 0;
	rc = llapi_parse_size("17 17", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);

	size_units = 0;
	rc = llapi_parse_size("17hello", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);

	size_units = 0;
	rc = llapi_parse_size("17 b", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);

	size_units = 0;
	rc = llapi_parse_size("-19", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);
...
#if 0
	size_units = 0;
	rc = llapi_parse_size("", &size, &size_units, 1);
	ck_assert_int_eq(rc, -1);
#endif

	/* Hex and octal */
	size_units = 0;
	rc = llapi_parse_size("0x17", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
	ck_assert_int_eq(size, 0x17);
	ck_assert_int_eq(size_units, 1);

	size_units = 0;
	rc = llapi_parse_size("017", &size, &size_units, 1);
	ck_assert_int_eq(rc, 0);
